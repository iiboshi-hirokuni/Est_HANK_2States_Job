...
gg = vars(n_v+1:n_v+n_g-3) + vars_SS(n_v+1:n_v+n_g-3);
azdeltavec = reshape(azdelta,I*J,1);
g_end = (1 - sum(gg .* azdeltavec(1:end-1))) ./ azdeltavec(end) ;
g = [gg;g_end];
MP = vars(n_v+n_g-2,1) + vars_SS(n_v+n_g-2);
FP = vars(n_v+n_g-1,1) + vars_SS(n_v+n_g-1);
d_TFP = vars(n_v+n_g,1) + vars_SS(n_v+n_g);
...
r_nom = vars(n_v+n_g+6) + vars_SS(n_v+n_g+6);

assets = assets - FP;

VDot = vars(nVars+1:nVars+n_v-1);
inflationDot = vars(nVars+n_v);
gDot = vars(nVars+n_v+1:nVars+n_v+n_g-3);
MPDot = vars(nVars+n_v+n_g-2);
FPDot = vars(nVars+n_v+n_g-1);
TFPDot = vars(nVars+n_v+n_g);

VEErrors = vars((2*nVars+1):(2*nVars+n_v-1),1);
inflationEError = vars(2*nVars+n_v,1);

MPShock = vars(2*nVars+n_v+1,1);
FPShock = vars(2*nVars+n_v+2,1);
TFPShock = vars(2*nVars+n_v+3,1);
...
lumptransfer = labtax * w * hours - Gov - (r_Nominal - (1-govbcrule_fixnomB) * inflation) * assets;
...
% Fiscal Policy
FPResidual = FPDot - (-ttheta_FP * FP + ssigma_FP * FPShock);

% TFP
TFPResidual = TFPDot - (-ttheta_TFP * d_TFP + ssigma_TFP * TFPShock);
